...
int main()
{
	...
	// slot number to use
	starforge::PipelineParam *param = pipeline->GetParam("uTextureSampler");
	if (param)
		param->SetAsInt(0);
	...
	starforge::VertexElement vertexElements[] = {
		{ 0, starforge::VERTEXELEMENTTYPE_FLOAT, 3, sizeof(Vertex), 0 },
		{ 1, starforge::VERTEXELEMENTTYPE_FLOAT, 2, sizeof(Vertex), 12 }
	};
	starforge::VertexDescription *vertexDescription = renderDevice->CreateVertexDescription(COUNT_OF(vertexElements), vertexElements);
	...
	starforge::IndexBuffer *indexBuffer = renderDevice->CreateIndexBuffer(sizeof(indices), indices);

	// create texture
	starforge::Texture2D *texture2D = renderDevice->CreateTexture2D(BMPWIDTH, BMPHEIGHT, image32);
